...
import FloatingMortgageAssistant from "@/components/FloatingMortgageAssistant";
      ...
      <main className="max-w-[1180px] mx-auto px-4">
        ...
        <section id="mortgage-calculator" className="mt-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8 items-start">
            {/* Inputs card */}
            <div className="bg-white border border-gray-200 rounded-md shadow-[0_1px_3px_rgba(0,0,0,0.08)] p-5 space-y-4">
              <Field label="Home Price" tip="The total purchase price of the home you plan to buy, before any down payment or financing.">
                <div className="flex items-center border border-gray-300 rounded-sm bg-white h-10 px-3">
                  <span className="text-gray-700 text-sm">$</span>
                  <input type="number" value={price} onChange={(e) => setPrice(+e.target.value || 0)} className="w-full pl-1 text-sm outline-none" />
                </div>
              </Field>
              <Field label="Down Payment" tip="The cash you pay upfront. A larger down payment lowers your loan amount and may help you avoid PMI.">
                <div className="flex gap-2">
                  <div className="flex items-center border border-gray-300 rounded-sm bg-white h-10 px-3 flex-1">
                    <span className="text-gray-700 text-sm">$</span>
                    <input type="number" value={down} onChange={(e) => setDown(+e.target.value || 0)} className="w-full pl-1 text-sm outline-none" />
                  </div>
                  <div className="flex items-center border border-gray-300 rounded-sm bg-white h-10 px-3 w-20">
              ...
              <Field label="Loan Term" tip="The number of years you have to repay the mortgage. Shorter terms usually mean higher monthly payments but less total interest.">
                <div className="grid grid-cols-4 border border-gray-300 rounded-sm overflow-hidden">
                  {[10, 15, 20, 30].map((y, i) => (
                    <button
                      key={y}
                      onClick={() => setYears(y)}
                      className={`h-10 text-[12px] ${i > 0 ? "border-l border-gray-300" : ""} ${years === y ? "bg-blue-700 text-white font-semibold" : "bg-white text-gray-700"}`}
                    >
                      {y} years
                    </button>
                  ))}
              ...
              <Field label="Mortgage Rate" tip="The annual interest rate charged on your loan. Even small changes can meaningfully affect your monthly payment.">
                <div className="flex items-center border border-gray-300 rounded-sm bg-white h-10 px-3">
              ...
              <div className="pt-1 space-y-2">
                <a className="block text-[13px] text-blue-700 font-semibold hover:underline cursor-pointer">Check current rates</a>
                {!showOptional && (
                  <button
                    onClick={() => setShowOptional(true)}
                    className="text-[13px] text-blue-700 font-semibold flex items-center gap-1 hover:underline"
                  >
                    More Optional Inputs <ChevronDown size={14} />
                  </button>
                )}
              </div>
              {showOptional && (
                <div className="space-y-4 pt-2">
                  <Field label="Property Tax (optional)" tip="Local taxes assessed by your city or county. Enter as an annual percentage of home price.">
                    <div className="flex items-center border border-gray-300 rounded-sm bg-white h-10 px-3">
                      <input
                        type="number"
                        step="0.01"
                    ...
                    <a className="block text-[13px] text-blue-700 font-semibold mt-1 hover:underline cursor-pointer">See Average Per State</a>
                  </Field>
                  <Field label="Homeowner Insurance per month (optional)" tip="Premium for a homeowner's insurance policy, typically paid monthly through escrow.">
                    <div className="flex items-center border border-gray-300 rounded-sm bg-white h-10 px-3">
                  ...
                  <Field label="PMI per month (optional)" tip="Private Mortgage Insurance is typically required when your down payment is less than 20%.">
                    <div className="flex items-center border border-gray-300 rounded-sm bg-white h-10 px-3">
                      <span className="text-gray-700 text-sm mr-1">$</span>
                      <input
                        type="number"
                  ...
                  <Field label="HOA per month (optional)" tip="Homeowners Association fees charged by your community for shared amenities and maintenance.">
                    <div className="flex items-center border border-gray-300 rounded-sm bg-white h-10 px-3">
                      <span className="text-gray-700 text-sm mr-1">$</span>
                      <input
                        type="number"
                  ...
                  <button
                    ...
                    className="text-[13px] text-blue-700 font-semibold flex items-center gap-1 hover:underline"
                  >
                    Hide Optional Inputs <ChevronUp size={14} />
                  </button>
                </div>
              )}
            </div>
            ...
            {/* Results */}
            <div>
              <h3 className="font-serif text-[22px] font-bold text-gray-900 mb-4">Payment Details</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-2">
                <Stat label="Monthly Payment" v={`$${fmt(total)}`} tip="The scheduled monthly payment of principal and interest on a mortgage, plus estimated taxes and insurance." />
                <Stat label="Total Interest Paid" v={`$${fmt(monthly * years * 12 - principal)}`} tip="The total amount of interest you would pay over the full life of the loan." />
                <Stat label="Total Cost of Loan" v={`$${fmt(monthly * years * 12)}`} tip="The sum of all principal and interest payments over the life of the loan." />
                <Stat label="Loan Payoff Date" v={`May ${2026 + years}`} tip="The estimated date your mortgage will be fully paid off if you make every scheduled payment on time." />
              </div>
              <a className="text-[13px] text-blue-700 inline-block mb-3 hover:underline">See Ledgerly Best Mortgage Lenders &raquo;</a>
              ...
              <a className="inline-block mt-5 text-[12px] font-bold text-blue-700 tracking-wider">VIEW MORTGAGE CALCULATOR &raquo;</a>
            </div>
          </div>
        </section>
